...
import { addTemplateItem } from '@/app/(dashboard)/companies/[id]/templates/actions'

type PddroPhase = 'P' | 'D' | 'DO' | 'R' | 'O'

export function AddTemplateItem({ companyId, phase }: { companyId: string; phase: PddroPhase }) {
  ...
  function handleAdd() {
    ...
    startTransition(async () => {
      await addTemplateItem(companyId, phase, name.trim(), formType)
      setName('')
      setOpen(false)
      router.refresh()
    })
  ...
  return (
    <div className="mx-5 px-3 py-2 bg-gray-50 rounded-lg flex items-center gap-2">
      ...
      <button onClick={() => { setOpen(false); setName('') }} className="text-xs text-gray-400 hover:text-gray-600 px-1">
        取消
      </button>
    </div>
  )
